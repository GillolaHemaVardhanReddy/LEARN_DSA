...
> `dashboard/PROGRESS.md (leak board)`).

---

## 🛫 GATE A — before you write any code (and BEFORE you name any pattern)
Say these out loud:
1. **Restate** the problem in ONE sentence, in your own words, **in exact units** (what is returned? a distance, a count, an index?). (Not the title — the actual ask.)
2. **3-element dry run** — make up a tiny input, hand-run what the answer should be.
3. **⭐ The 4 disqualifier questions — answered about the PROBLEM, before any tool is named (M#12):**
   **contiguous or not?** (subarray vs subsequence/subset) · **sorted, or may I sort?** (2-pointer vs hashing) ·
   **does ORDER matter, or only MEMBERSHIP?** (loop + `used[]` vs pick/not-pick) · **COUNT, BEST, or LIST-ALL?**
   (counting math vs greedy/DP vs enumeration). Only after all four does a tool get named.
> *Why:* your reading misses (MISTAKE #10) happen when you skip 1–2 under pressure; and on 2026-08-10 you
> answered three questions in a row with the *previous* question's tool ("here also its pick not pick") —
> the gate interrogates the problem, so the last question can't contaminate it. **Watch-tell:** any answer
> that starts with "here also…" / "this is the same as…" → stop, run the gate.
...
how the ritual earns trust — and it feeds the leak scoreboard in `dashboard/PROGRESS.md (leak board)`.
