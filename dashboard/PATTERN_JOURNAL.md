...
>
> **v2 (2026-09-07) — two standing rules.** (1) Every cue ends with **"reduces to ___"** — which earlier
> tool this is built from (PLAN §2 rule 5: knowledge used inside later material becomes permanent).
> (2) Every entry doubles as a **card**: *front* = a disguised statement with 2–3 examples · *back* = trigger
> + core idea + why + complexity-with-the-work-in-it, in boss's words, **no code** (Wozniak: minimum
> information). Cards are re-fired cold and MIXED via `REVISION_QUEUE.md` — never re-read (a cue that only
> fires on-topic isn't banked: the subarray-vs-subsequence cue written 7/14 did not fire on 8/10).
...
Taught me by: LC232 (two-stack FIFO, amortized O(1)) + LC933 (front-expiry queue) mechanics → **LC239 gate (judge-AC + stress-green) = L3**. Striver [297][312]. **Practice queued** (`practice/08-queues-deque/reps/`): LC622 ring buffer · LC1438 two-deque · LC862 deque-over-prefix (unlocks the parked hard) · LC1696 deque-over-DP.
