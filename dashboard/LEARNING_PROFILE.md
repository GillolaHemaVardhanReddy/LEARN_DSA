...
### 2026-09-07 — v2: the meta-learning LAB is now formal (`dashboard/SYSTEM.md` §7)
Boss's ask: *"let me learn how I learn, make the system learn which way I learn best — it should experiment all
ways and decide which is best for me."* The **learner-config table** in SYSTEM.md §7a is now the source of truth
for what lands (confirmed / likely / untested), this file stays the dated evidence log. Experiments run one at a
time, named out loud when they start, reviewed at the Friday checkpoint, adopted only if the **+7 d cold re-solve
rate** is higher AND he prefers it; he can veto any. Queued for Tier 0: **E1** seeded BUILD statements ·
