...
| **2026-07-04** | **LC149 Max Points — gcd-canonical KEY, cold re-derive (+1d)** | +1d | solo | 🏆 JUDGE-AC 7/03. Closed-book: WHY anchor (fixed point turns "same slope" → "same line", not parallel)? WHY gcd (strips magnitude, keeps direction — `(2,1)(4,2)(6,3)`→one key)? WHY a `(dy,dx)` PAIR not a float (float truncates/collides; a 1-D key can't carry a 2-D direction)? `gcd(0,x)=x` so horiz/vert need NO special case; `g==0` iff self-pair. Trigger to recall cold: **"key is a ratio/direction → canonicalize (gcd + sign) before hashing."** Pass → +3d. |
