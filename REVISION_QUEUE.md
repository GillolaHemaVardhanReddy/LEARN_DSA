...
|---|---|---|---|---|
| **2026-06-24 (morning)** | **LC739 Daily Temperatures — SUBMIT in browser → confirm judge-AC** | judge | solo | Code is stress-GREEN (100k + canonical) but **not yet judge-AC** (MCP `isSignedIn:false` this session → 403). **Restart Claude Code so MCP signs in**, then submit. The browser link is in `learn/07-Stacks/02-Daily-Temperatures/problem.md`. Confirming AC = first half of M7→L4. |
| **2026-06-24** | **M7 L4 reps — NGE II (LC503) + Stock Span (LC901) solo** | solo | solo | Both scaffolded/named (`learn/07-Stacks/03-Next-Greater-Element-II/`). NGE II twist = **circular** (loop `i` 0→2n-1, read `nums[i%n]`, store value, 2nd lap only RESOLVES, doesn't push). Stock Span (LC901) = **previous-greater** + count a span (engine flipped). These 2 solo + DT judge-AC = **M7 L4 earned**. |
| **2026-06-24** | **Monotonic stack — concept recall (+1d)** | +1d | recall | L3 earned 6/23. Closed-book: the **recording rule** (pre-fill `ans(n,default)` + `ans[st.top()]=...`, never push_back — why?) + WHEN next-greater wants a DECREASING stack + the O(n) amortization in one line ("push once, pop once → 2n"). |
