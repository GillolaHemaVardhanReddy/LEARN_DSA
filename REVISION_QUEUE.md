...
| **next session** | **Remaining stacks practice (interleaved keep-warm) + Stock Span** | reps | solo | `practice/06-Stacks-Monotonic/`: **LC1004 Max Consecutive Ones III** (= MISTAKE #2 re-test, variable SW longest, ≤k zeros) · **LC33 Search in Rotated Sorted Array** (open BS leak — discard-ability + "could mid be the answer?") · plus **LC901 Stock Span** (previous-greater + span count) for monotonic depth. |
