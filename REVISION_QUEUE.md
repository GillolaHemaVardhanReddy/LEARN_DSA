...
|---|---|---|---|---|
| **2026-07-05** | **M8 Monotonic Deque (LC239) — concept recall (+1d)** | +1d | recall | L3 earned 7/04 (judge-AC + stress-green). Closed-book: (1) why store **indices not values** (front-index vs `i-k` detects expiry — a value can't; killed by `[5,3,1,2],k=3`)? (2) exact expire edge = `<= i-k` not `<`? (3) why front is always the max (decreasing invariant) and why O(n) amortized (each index enters/leaves once)? Pass → +3d. |
