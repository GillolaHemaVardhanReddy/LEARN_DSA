...
|---|---|---|---|---|
| **MORNING (2026-06-23)** | **Monotonic stack — theory FRESH, then Daily Temperatures (LC739) solo** | new (L2→L4 rep) | recall+solo | He asked to learn the theory fresh in the morning (decayed over ~12d). FIRST closed-book: state the pattern in his words (kept-sorted stack of INDICES, "elements waiting for their answer," pop-resolves-on-break, O(n) amortized = push-once/pop-once). THEN he picks the knobs for LC739 himself (direction? order? record what on pop?) and codes the optimal — **I owe him the derivation; do NOT re-spoil it** (I already over-revealed the Daily-Temps trace 6/22). Scaffold ready at `learn/07-Stacks/02-Daily-Temperatures/`. AC = M7 → L4. |
