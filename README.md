...
A self-contained DSA learning system run by **Tommy**, your mentor. Claude Code reads
...
On the **first** launch, `PROGRESS.md` is empty, so Tommy runs a short diagnostic and
...
## Daily loop
- **Start:** run `claude`, then type `/continue`. Tommy reads your state, shows the
...
- **End:** type **`/endsession`**. This is the one habit that matters — it writes your
  progress to the files and commits. (Tommy also updates files as you go, but
...
2. **"Solved" = accepted on LeetCode**, not "looks right." Tommy can run code locally,
