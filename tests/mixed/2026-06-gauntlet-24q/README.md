...
> **`practice/<topic>/hard/`** (all Phase-1 hards now live in one place). This drill is now
...
## WHY THIS MIX (Kira's calibration call)
...
| 6 | Hashing (frequency count) | ~ (got "hashmap"; restate misread as substring; presence-vs-count gap) | ~ | presence-loop O(n²) | ✓ AC | Restate drifted to "substring" (wrong — order/contiguity irrelevant; counterexample ab/ba). Brute checked PRESENCE not COUNTS — same trap as anagram Q7 (recurring presence-vs-frequency). Optimal = freq count `int[26]`, magazine ≥ note. (Kira OVER-REVEALED the optimal — protocol breach, noted.) Placed on hashing sub-pattern map: frequency-count flavor. |
...
> `dashboard/MISTAKES.md` with root cause + re-test. Pass Drill 1 well → Drill 2 gets harder.
