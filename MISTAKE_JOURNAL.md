...
| **PREMATURE "DONE" — fixes 1 bug, declares finished / asks "check" before self-tracing the rest** | **4 (all 2026-06-23)** | The self-Gate-C leak: after fixing ONE bug he reaches for external validation ("check now") instead of re-running the check on his WHOLE code. STANDING RULE: before "done," **trace your own current code on a hostile input** (one where the bug shows, e.g. position≠distance) and read the output. Antidote confirmed: his trace-strength aimed inward finds the bugs. Tell HIM: *wanting to ask "is it ok" IS the cue to trace it yourself.* Clears on a multi-bug solve where he self-traces to clean before asking. | 2026-06-23 (NGE ×1, Daily Temps ×3) |
