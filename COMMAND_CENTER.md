...
- **Current topic / workspace:** **M8 Queues & Deque — `phase-2/learn/08-Queues-Deque/`.** Clean per-problem folders built (each = `question.md` + `solution.cpp`). ✅ **LC232 Queue-using-Stacks DONE** (dodged with an array-queue first → redirected to the real two-stack technique → upgraded to **amortized O(1)**, killed his own pour-back redundancy). ✅ **LC933 Recent Calls DONE** (O(N²) brute → **front-expiry queue O(1) amortized**; derived "increasing t ⇒ expired-forever" himself). **Empty-container UB leak caught+logged** (believed `.back()` on empty returns 0 → it's UB; CPP #9 / mistakes M#1). **NEXT: `03-LC239 Sliding Window Maximum` — the monotonic-deque GATE** (fuses pop_front expiry + pop_back monotonic invariant). Restate + brute oracle first, then optimal + stress. Clears parked LC862. **M8 now L2** (mechanics coded+understood; LC239 = the L3 gate).
