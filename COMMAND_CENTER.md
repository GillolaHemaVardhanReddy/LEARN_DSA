...
- **Date:** 2026-07-03 (**🏆 FIRST HARD DOWN — LC149 Max Points JUDGE-AC**)
- **Current topic / workspace:** **HARD GAUNTLET P1 — LC149 Max Points on a Line ✅ JUDGE-AC.** Anchor + slope-hash O(n²), **derived the whole arc cold** (brute line-eq → O(n³) → slope-from-fixed-point → **gcd-canonical (dy,dx) pair**). Killed the float-key trap by tracing his OWN hostile inputs. **Gate 3 fired unprompted twice** (built `[[4,5],[4,-1],[4,0]]` vertical + `[[0,0],[1,1],[0,5]]` vertical-vs-slope-1 himself). Teach-it-back + calibration both banked. C++ learned: `map` needs `<` / `unordered_map` needs a hash (no default for pair); `gcd(0,x)=x`. **NEXT: gauntlet P2 — LC42 Trapping Rain Water (two-pointer) or LC84 Largest Rectangle (stacks).**
