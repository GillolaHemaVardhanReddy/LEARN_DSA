...
- **Date:** 2026-07-06 (**M9 practice Q3 LC50 Pow(x,n) — brute done + INT_MIN lesson banked; ⏸️ PAUSED mid-problem, resumes ~2h**)
- **🎯 RESUME POINT (exact):** `phase-2/practice/09-Recursion/03-LC50-Powx-n/solution.cpp` → write the **O(log n) `myPow`** (base case e==0→1 · half=myPow(x,e/2) · half*half · odd leftover ×x · negative→1/x^|n| via widened long long) → submit to LC. His own bridge words are already in the file: *"O(n)→O(log n) collapse because each recursion halves n."* Brute ✅ correct (widen-then-negate INT_MIN fix, HIS edit) — it can never pass LC by design (2.1e9 frames); that lesson is the point.
- **Leaks fired today:** **#8 overflow, new costume** — `abs(n)` on an int at INT_MIN = negation in the narrow type (→ `phase-2/mistakes.md` M#4, scoreboard reset). Plus a model gap now logged as **M#5 verdict literacy**: fixed logic but read the red TLE as "logic still wrong" — WA=logic vs TLE/RE-at-max-n=scale-wall distinction taught.
- **⭐ KIRA WORKFLOW RULE (his explicit call-out today):** boss runs code ONLY on LeetCode, never locally — diagnose every "wrong ans" from the LC verdict, never from local build mechanics (→ LEARNING_PROFILE).
- **M9 practice ladder:** Q1 LC1137 Tribonacci ✅ AC · Q2 LC231 Power-of-Two ✅ AC both ways · **Q3 LC50 ← HERE (optimal owed)** · Q4 LC746 Min-Cost-Stairs · Q5 LC198 House Robber. **M9 recall +1d was due today — still owed**, run it at session open before Q3 (closed-book: leap-of-faith + two shapes + memo-why).
...
| M9 Recursion | **L3** | [008-012] | ladder (print/sum/factorial/reverse/palindrome/fib) · LC70 · **LC1137 ✅ · LC231 ✅** | **LC50 optimal owed (resume point)**; #8 overflow fired 7/06 (abs-at-INT_MIN, M#4) + verdict-literacy M#5; recall +1d OVERDUE (was 7/06) | finish LC50 → LC746 → LC198; ≥2 medium ACs solo → L4 |
