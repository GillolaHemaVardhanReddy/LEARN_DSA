...
Prevention rule: **BEFORE submitting, trace the 4 edges myself** — single element (present/absent), empty, target < all / > all (insert 0 / n), target at first/last index. No `if(s==e)` patches. Trust the template. (In phase-1/notes/00 boundary framework + phase-1/notes/06.)
