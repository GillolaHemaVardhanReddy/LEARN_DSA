...
- **Dose (D1, month 1):** **45 min standard · cap 60** — RECALL 10 · BUILD 30 · LOOK BACK 5. Review 2026-10-07 → 60–90 if ≥ 20 of 30 days kept. **Floor = one 10-min card. Zero is the only failure.**
...
| `LOG.md` | append-only session log (+ frozen v1 history). Read only the latest 2 entries. |
...
1. Read `README.md` TODAY → `REVISION_QUEUE.md` due rows → `PROGRESS.md` (head). Print **4 lines**: *why today matters* (today → Friday checkpoint → tier gate → month-7), days kept x/7 · n/66, what's due, next contest. **No dashboard wall.**
2. **RECALL (10 min)** — 1–2 due cards / cold re-solves, always a **different topic** than today's BUILD. Marks only.
3. **BUILD (30 min)** — **ONE problem** in the current topic. Gate A first: restate in exact units + 3-element dry run + the **4 disqualifier questions** *before any tool is named*. LEARN = brute → bridge → optimal · REP = solve → LC → look back. A second problem only if the first was quick.
...
5. **At the cap, STOP** — even mid-problem; write the resume line into README TODAY. Enforcing the cap is Kira's job too.
Week: Mon–Thu build · **Fri `/checkpoint`** · Sat one hard or the biweekly · **Sun LC Weekly 08:00 IST → `/contest`** (log + one upsolve). Every statement Kira hands him **ships with 2–3 concrete input→output examples**.
...
- **Habit protocol:** 5 of 7 = kept week · 1 repair token / week · one missed day = noise · two missed → next session opens with ONE 5-min card, no backlog, no guilt · missed week → 30-min cold checkpoint on the current topic, then continue · queue > 3 due ⇒ prune, never pile.
- **Contests are the use, not the training.** After each: exactly one upsolve (≤ 1 hint) + one card per unsolved. Log `CONTESTS.md`.
- **Meta-learning lab** (`SYSTEM.md` §7): one named experiment at a time, never silent, logged to `LEARNING_PROFILE.md`; he can veto.
- **Levels only on witnessed evidence** (PLAN §8): L1 derived · L2 explained + traced · L3 template from memory + 1 LEARN AC · L4 Friday checkpoint + ≥ 2 mediums solo · L5 named cold ≥ 14 d later in a mixed test *or* used in a contest · L6 taught back. % map L0=0 L1=15 L2=35 L3=55 L4=75 L5=90 L6=100. v1 levels are provisional (`*`) until their Tier-0 checkpoint. Advance at L3; a tier opens when every topic in the previous tier is L4.
