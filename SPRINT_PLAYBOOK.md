# SPRINT_PLAYBOOK.md — How Hema Vardhan & Kira Run Each Topic
...
- Kira teaches **just-in-time**, no bloated lecture: why it exists → intuition +
...
- Writing it yourself = the encoding step. Kira only corrects, never pre-fills.
...
- Kira gives problems **one at a time**, easy → medium, via the **Problem-Solving Loop**:
  Restate → Constraints/edge cases → Brute force + complexity → Bottleneck →
  Pattern hunt → Optimal → Complexity → Implement → Test.
- **Hint ladder** (Hema asks; Kira never front-runs it):
