/*
================================================================
  DRILL 1  —  Problem 28   [medium]
================================================================
  No two adjacent elements are equal. Find ANY peak — an element strictly greater than both neighbors (beyond the ends is -infinity). O(log n).

  Example:     [1,2,1,3,5,6,4]  ->  index of 6 (or 1)
  Constraints: 1 <= n <= 1000

  ----------------------------------------------------------------
  BEFORE YOU CODE (in order — this IS the drill):
    1. RESTATE the problem in ONE plain sentence.
    2. DRY-RUN the example above by hand.
    3. CONSTRAINTS  ->  target complexity?
    4. NAME THE PATTERN. Gates to say out loud:
         - "Is the data SORTED?"  no  -> probably hashing, not two pointers.
         - "Any NEGATIVES / a +-1 transform?"  yes -> prefix+hash, not a window.
    5. Find this problem on LeetCode (LINKS.md has the link), then code below & SUBMIT.
    6. Boundary checklist before submit: empty / single / first&last / overflow.
  ----------------------------------------------------------------
  My restatement     :
  My pattern guess   :
  My target Big-O    :
================================================================
*/

#include <bits/stdc++.h>
using namespace std;

class Solution {
public:
    // TODO: copy the exact method signature from the LeetCode link in LINKS.md,
    //       then implement it here.

};

int main() {
    // Optional: wire up the example above as a local sanity check before submitting.
    return 0;
}
