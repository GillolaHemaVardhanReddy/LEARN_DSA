...
| M6 | Binary Search | **L3** | 2026-06-09 | 2/0 | **L3 (implements)**: theory + videos + recognition map (exact/bounds/**search-on-answer** Koko/rotated/2D), 3 templates + store-candidate + pitfalls (Notes/06). **AC: LC704 (exact), LC35 (lower bound)** — coded from understanding. ⚠️ but via **reactive band-aid `if(s==e)` patches** + debugging-by-failing-test (not trusting the clean template). MEDIUMS (LC34/875) = tomorrow. Clean L4 once mediums done w/ the template + edges reasoned UP FRONT. |
