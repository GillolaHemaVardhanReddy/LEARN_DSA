...
- Current topic: **M3 Two Pointers — L4** (converging earned via LC167 clean solo; GAPS: Dutch-flag partition unpracticed [LC75 done via sorting], boundaries still leak, L5 pending deferred drill). **Now moving to M6 Binary Search PROBLEMS** (theory+videos done, ladder built at practice/05-Binary-Search/). CADENCE (memory): weekday=L4 (easy+medium), HARDS=weekend, L5 deferred to spaced drills. Mastered L4: SW, Prefix, Hashing, Two Pointers. REVISIT targets: 2ptr Dutch-flag (LC75) + boundary reflex. Execution/boundary still #1 leak (12×).
...
| M6 | Binary Search | **L1** | 2026-06-09 | 0/0 | **L1 (introduced)**: theory + Striver videos DONE. Halving/O(log n), MONOTONICITY (NO→YES), recognition map (exact/bounds/**search-on-answer** Koko/rotated/2D), 3 templates + store-candidate + pitfalls (Notes/06). Ladder built (practice/05-Binary-Search/, MCP-verified). **Problems PENDING — after the 2ptr solo set** (his sequencing). |
